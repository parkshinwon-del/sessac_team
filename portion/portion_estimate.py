...
train_transform = transforms.Compose([
    transforms.RandomResizedCrop(224, scale=(0.6, 1.0)),   # 확대/축소 대응 (이미 추가하신 것)
    transforms.RandomHorizontalFlip(),
    transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3),  # 조명/화이트밸런스 대응
    transforms.RandomRotation(15),                          # 카메라 기울기 대응
    transforms.ToTensor(),
    transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    transforms.RandomErasing(p=0.3, scale=(0.02, 0.1)),     # 사진 일부를 랜덤하게 가림
])
